...
data = []
for j in range(1, 6):
    url = 'https://piao.qunar.com/ticket/list.htm?keyword=%E7%8E%89%E6%BA%AA&region=&from=mps_search_suggest&page='+str(i)
    req = requests.get(url, headers = headers)
    soup = BeautifulSoup(req.text, 'lxml')

    names = soup.find_all('a', class_ = 'name')
    areas = soup.find_all('span', class_ = 'area')
    hots = soup.find_all('div', class_ = 'sight_item_hot')
    address = soup.find_all('p', class_ = 'address color999')
    intros = soup.find_all('div', class_ = 'intro color999')


    for n,a,h,ad,i in zip(names,areas,hots,address,intros):

        data.append([n.text, a.text, h.find('em').text, ad.find('span').text, i.text])
        
D = pandas.DataFrame(data, columns=['Name', 'Area', 'Hot', 'Address', 'Introduce'])
D.to_csv('data.csv')